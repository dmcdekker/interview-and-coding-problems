...
class Node(object):
    ...
    def count_employees(self):
        ...
        """
        count = 0
        # loop through all children & add count while calling recursively on child
    ...
    def count_employees_nonrecursive(self):
        ...
        """
        count = 0
        # pass employee tree in as list
        to_visit = [self]

        # while there are existing children/nodes 
        while to_visit:
            # pop last last node to add child
            emp = to_visit.pop()
            # for each child incr count and add to to_visit
